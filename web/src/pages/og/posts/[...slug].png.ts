...
import {
  OG_W, OG_H, fontData,
  loadAdditionalAsset, renderToPng, pngResponse,
} from '~/lib/og-utils';
...
async function generateFallbackHero(title: string): Promise<Buffer> {
  ...
  const svg = await satori(
      ...
      props: {
        ...
        children: [
          ...
          { type: 'div', props: { style: { position: 'absolute', bottom: '-40px', left: '-40px', width: '200px', height: '200px', borderRadius: '50%', background: `hsl(${hue},50%,60%)`, opacity: 0.08 } } },
          {
            type: 'div',
            props: {
              style: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' },
              children: [{
                type: 'span',
                props: { style: { fontSize: '40px', fontWeight: 700, color: `hsl(${hue},40%,30%)`, textAlign: 'center', lineHeight: 1.4 }, children: title },
              }],
            },
          },
        ],
    ...
    { width: OG_W, height: HERO_H, fonts: [{ name: 'ArkPixel', data: fontData, style: 'normal' as const }], loadAdditionalAsset },
  );
...
async function buildHeroDataUri(imgBuffer: Buffer): Promise<string> {
  ...
  const blurBg = await sharp(imgBuffer).resize(OG_W, HERO_H, { fit: 'cover' }).blur(30).jpeg({ quality: 60 }).toBuffer();
  const fg = await sharp(imgBuffer).resize(OG_W, HERO_H, { fit: 'inside' }).toBuffer();
  const fgMeta = await sharp(fg).metadata();
  const composite = await sharp(blurBg)
    .composite([{ input: fg, left: Math.round((OG_W - fgMeta.width!) / 2), top: Math.round((HERO_H - fgMeta.height!) / 2) }])
  ...
  return await Promise.all(posts.map(async (post) => {
    ...
    try {
      if (post.data.heroImage) {
        ...
        if (fsPath) heroDataUri = await buildHeroDataUri(readFileSync(fsPath));
      } else {
    ...
    props: {
      ...
      children: [
        ...
        },
        {
          type: 'div',
          props: {
            style: { width: '100%', height: '45%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '30px 50px', background: barBg },
            children: [
